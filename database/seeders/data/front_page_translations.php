<?php

return [
'About Our University' => [
        'ar' => 'عن جامعتنا',
    ],
    'About Instructor' => [
        'en' => 'About Instructor',
        'ar' => 'عن المدرب',
    ],
    'About Instructors' => [
        'en' => 'About Instructors',
        'ar' => 'عن المدربين',
    ],
    'Accepted formats: PDF, DOC, DOCX, TXT, ZIP, RAR' => [
        'ar' => 'الصيغ المقبولة: PDF، DOC، DOCX، TXT، ZIP، RAR',
    ],
    'Add Answer' => [
        'ar' => 'إضافة إجابة',
    ],
    'Add New Question' => [
        'ar' => 'إضافة سؤال جديد',
    ],
    'Admin' => [
        'ar' => 'المشرف',
    ],
    'Already Submitted' => [
        'ar' => 'تم الإرسال مسبقاً',
    ],
    'Already have an account?' => [
        'en' => 'Already have an account?',
        'ar' => 'لديك حساب بالفعل؟',
    ],
    'Login' => [
        'en' => 'Login',
        'ar' => 'تسجيل الدخول',
    ],
    'Sign in' => [
        'en' => 'Sign in',
        'ar' => 'تسجيل الدخول',
    ],
    'Sign up' => [
        'en' => 'Sign up',
        'ar' => 'إنشاء حساب',
    ],
    "Don't have an account?" => [
        'en' => "Don't have an account?",
        'ar' => 'ليس لديك حساب؟',
    ],
    'Email address' => [
        'en' => 'Email address',
        'ar' => 'البريد الإلكتروني',
    ],
    'Password' => [
        'en' => 'Password',
        'ar' => 'كلمة المرور',
    ],
    'Remember me' => [
        'en' => 'Remember me',
        'ar' => 'تذكرني',
    ],
    'Create an Account' => [
        'en' => 'Create an Account',
        'ar' => 'إنشاء حساب',
    ],
    'Name' => [
        'en' => 'Name',
        'ar' => 'الاسم',
    ],
    'Country' => [
        'en' => 'Country',
        'ar' => 'الدولة',
    ],
    'Confirm Password' => [
        'en' => 'Confirm Password',
        'ar' => 'تأكيد كلمة المرور',
    ],
    'Forgot Password' => [
        'en' => 'Forgot Password',
        'ar' => 'نسيت كلمة المرور',
    ],
    "I don't remember my password" => [
        'en' => "I don't remember my password",
        'ar' => 'لا أتذكر كلمة المرور',
    ],
    'Enter your email and we will send you a 6-digit OTP to reset your password.' => [
        'en' => 'Enter your email and we will send you a 6-digit OTP to reset your password.',
        'ar' => 'أدخل بريدك الإلكتروني وسنرسل لك رمز التحقق المكون من 6 أرقام لإعادة تعيين كلمة المرور.',
    ],
    'Send OTP' => [
        'en' => 'Send OTP',
        'ar' => 'إرسال رمز التحقق',
    ],
    'Back to login' => [
        'en' => 'Back to login',
        'ar' => 'العودة لتسجيل الدخول',
    ],
    'Reset Password' => [
        'en' => 'Reset Password',
        'ar' => 'إعادة تعيين كلمة المرور',
    ],
    'Enter the 6-digit OTP sent to your email and your new password.' => [
        'en' => 'Enter the 6-digit OTP sent to your email and your new password.',
        'ar' => 'أدخل رمز التحقق المكون من 6 أرقام المرسل لبريدك الإلكتروني وكلمة المرور الجديدة.',
    ],
    'OTP Code (6 digits)' => [
        'en' => 'OTP Code (6 digits)',
        'ar' => 'رمز التحقق (6 أرقام)',
    ],
    'New Password' => [
        'en' => 'New Password',
        'ar' => 'كلمة المرور الجديدة',
    ],
    'Request new OTP' => [
        'en' => 'Request new OTP',
        'ar' => 'طلب رمز تحقق جديد',
    ],
    'Verify OTP' => [
        'en' => 'Verify OTP',
        'ar' => 'التحقق من رمز التحقق',
    ],
    'Enter the 6-digit OTP sent to your email.' => [
        'en' => 'Enter the 6-digit OTP sent to your email.',
        'ar' => 'أدخل رمز التحقق المكون من 6 أرقام المرسل إلى بريدك الإلكتروني.',
    ],
    'Email:' => [
        'en' => 'Email:',
        'ar' => 'البريد الإلكتروني:',
    ],
    'Set New Password' => [
        'en' => 'Set New Password',
        'ar' => 'تعيين كلمة مرور جديدة',
    ],
    'Enter your new password below.' => [
        'en' => 'Enter your new password below.',
        'ar' => 'أدخل كلمة المرور الجديدة أدناه.',
    ],
    'Password reset successfully. You can now log in with your new password.' => [
        'en' => 'Password reset successfully. You can now log in with your new password.',
        'ar' => 'تم إعادة تعيين كلمة المرور بنجاح. يمكنك الآن تسجيل الدخول بكلمة المرور الجديدة.',
    ],
    'Register' => [
        'en' => 'Register',
        'ar' => 'تسجيل',
    ],
    'Show details' => [
        'en' => 'Show details',
        'ar' => 'عرض التفاصيل',
    ],
    'Add to cart' => [
        'en' => 'Add to cart',
        'ar' => 'إضافة إلى السلة',
    ],
    'This email is already registered. Please sign in or use a different email.' => [
        'en' => 'This email is already registered. Please sign in or use a different email.',
        'ar' => 'هذا البريد الإلكتروني مسجل بالفعل. يرجى تسجيل الدخول أو استخدام بريد آخر.',
    ],
    'An error occurred during registration.' => [
        'ar' => 'حدث خطأ أثناء التسجيل.',
    ],
    'An error occurred while processing your registration. Please try again.' => [
        'ar' => 'حدث خطأ أثناء معالجة تسجيلك. يرجى المحاولة مرة أخرى.',
    ],
    'An error occurred while sending your message.' => [
        'en' => 'An error occurred while sending your message.',
        'ar' => 'حدث خطأ أثناء إرسال رسالتك.',
    ],
    'Answer submitted successfully!' => [
        'ar' => 'تم إرسال الإجابة بنجاح!',
    ],
    'Any additional information or questions' => [
        'ar' => 'أي معلومات أو أسئلة إضافية',
    ],
    'Any additional notes for your instructor...' => [
        'ar' => 'أي ملاحظات إضافية للمدرب...',
    ],
    'Artificial Intelligence is finally here and most of us are already act...' => [
        'ar' => 'لقد وصل الذكاء الاصطناعي أخيراً ومعظمنا بدأ بالفعل في التفاعل...',
    ],
    'Ask First Question' => [
        'ar' => 'اطرح السؤال الأول',
    ],
    'Ask anonymously' => [
        'ar' => 'اطرح بشكل مجهول',
    ],
    'Assignment' => [
        'ar' => 'واجب',
    ],
    'Assignment Content' => [
        'ar' => 'محتوى الواجب',
    ],
    'Assignment Help' => [
        'ar' => 'مساعدة الواجب',
    ],
    'Available Appointments' => [
        'ar' => 'المواعيد المتاحة',
    ],
    'Be the first to ask a question about this course!' => [
        'ar' => 'كن أول من يطرح سؤالاً حول هذه الدورة!',
    ],
    'By' => [
        'en' => 'By',
        'ar' => 'بواسطة',
    ],
    'Bestseller' => [
        'ar' => 'الأكثر مبيعاً',
    ],
    'Birth Date' => [
        'ar' => 'تاريخ الميلاد',
    ],
    'Brief description of the issue' => [
        'ar' => 'وصف مختصر للمشكلة',
    ],
    'Browse Courses by Category' => [
        'ar' => 'تصفح الدورات حسب الفئة',
    ],
    'Browse all assignments' => [
        'ar' => 'تصفح جميع الواجبات',
    ],
    'Browse questions' => [
        'ar' => 'تصفح الأسئلة',
    ],
    'Bundle Deals' => [
        'ar' => 'عروض الباقات',
    ],
    'Keep in Touch' => [
        'en' => 'Keep in Touch',
        'ar' => 'ابق على تواصل',
    ],
    'Get In Touch' => [
        'en' => 'Get In Touch',
        'ar' => 'تواصل معنا',
    ],
    'Call Now!' => [
        'en' => 'Call Now!',
        'ar' => 'اتصل الآن!',
    ],
    'Cancel' => [
        'ar' => 'إلغاء',
    ],
    'Capacity' => [
        'ar' => 'السعة',
    ],
    'Certificates' => [
        'ar' => 'الشهادات',
    ],
    'Class Title' => [
        'ar' => 'عنوان الفصل',
    ],
    'Close' => [
        'ar' => 'إغلاق',
    ],
    'Comments/Questions' => [
        'ar' => 'تعليقات/أسئلة',
    ],
    'Community name' => [
        'ar' => 'اسم المجتمع',
    ],
    'Completed' => [
        'ar' => 'مكتمل',
    ],
    'Congratulations!' => [
        'ar' => 'تهانينا!',
    ],
    'Content' => [
        'ar' => 'المحتوى',
    ],
    'Continue To Lecture' => [
        'ar' => 'متابعة إلى المحاضرة',
    ],
    'Course Content' => [
        'ar' => 'محتوى الدورة',
    ],
    'Course Video' => [
        'ar' => 'فيديو الدورة',
    ],
    'Course details' => [
        'ar' => 'تفاصيل الدورة',
    ],
    'Course detail' => [
        'ar' => 'تفاصيل الدورة',
    ],
    'What learn' => [
        'en' => 'What you will learn',
        'ar' => 'ماذا ستتعلم',
    ],
    'Courses' => [
        'ar' => 'الدورات',
    ],
    'Courses by' => [
        'ar' => 'دورات بواسطة',
    ],
    'Date' => [
        'ar' => 'التاريخ',
    ],
    'Describe your trading experience' => [
        'ar' => 'صف خبرتك في التداول',
    ],
    'Describe your training/teaching experience' => [
        'ar' => 'صف خبرتك في التدريب/التدريس',
    ],
'Description' => [
        'ar' => 'الوصف',
    ],
    'Course' => [
        'en' => 'Course',
        'ar' => 'دورة',
    ],
    'Site Name' => [
        'en' => 'Site Name',
        'ar' => 'اسم الموقع',
    ],
    'Designing' => [
        'ar' => 'التصميم',
    ],
    'Details' => [
        'ar' => 'تفاصيل',
    ],
    'Details of a fashion design course may include: Fundamentals of fashion...' => [
        'ar' => 'قد تتضمن تفاصيل دورة تصميم الأزياء: أساسيات الموضة...',
    ],
    'Digital Marketing Fundamentals' => [
        'ar' => 'أساسيات التسويق الرقمي',
    ],
    'Download File' => [
        'ar' => 'تنزيل الملف',
    ],
    'Download Materials' => [
        'ar' => 'تنزيل المواد',
    ],
    'Email Now' => [
        'en' => 'Email Now',
        'ar' => 'راسلنا الآن',
    ],
    'Due Date' => [
        'ar' => 'تاريخ الاستحقاق',
    ],
    'Due:' => [
        'ar' => 'الاستحقاق:',
    ],
    'Duration' => [
        'ar' => 'المدة',
    ],
    'Duration (minutes)' => [
        'ar' => 'المدة (بالدقائق)',
    ],
    'Email' => [
        'ar' => 'البريد الإلكتروني',
    ],
    'Email address' => [
        'ar' => 'عنوان البريد الإلكتروني',
    ],
    'Enter class description' => [
        'ar' => 'أدخل وصف الفصل',
    ],
    'Enter class title' => [
        'ar' => 'أدخل عنوان الفصل',
    ],
    'Enter homework description' => [
        'ar' => 'أدخل وصف الواجب',
    ],
    'Enter homework title' => [
        'ar' => 'أدخل عنوان الواجب',
    ],
    'Enter your assignment content here...' => [
        'ar' => 'أدخل محتوى واجبك هنا...',
    ],
    'Enter your homework content here...' => [
        'ar' => 'أدخل محتوى واجبك هنا...',
    ],
    'Enter your question title' => [
        'ar' => 'أدخل عنوان سؤالك',
    ],
    'Error downloading materials.' => [
        'ar' => 'حدث خطأ أثناء تنزيل المواد.',
    ],
    'Error joining live class.' => [
        'ar' => 'حدث خطأ أثناء الانضمام إلى الفصل المباشر.',
    ],
    'Error loading homework details.' => [
        'ar' => 'حدث خطأ أثناء تحميل تفاصيل الواجب.',
    ],
    'Error submitting answer' => [
        'ar' => 'حدث خطأ أثناء إرسال الإجابة',
    ],
    'Error submitting homework.' => [
        'ar' => 'حدث خطأ أثناء إرسال الواجب.',
    ],
    'Error submitting question' => [
        'ar' => 'حدث خطأ أثناء إرسال السؤال',
    ],
    'Explore by Category' => [
        'ar' => 'استكشف حسب الفئة',
    ],
    'Failed to join live class.' => [
        'ar' => 'فشل الانضمام إلى الفصل المباشر.',
    ],
    'Failed to load homework details.' => [
        'ar' => 'فشل في تحميل تفاصيل الواجب.',
    ],
    'Failed to submit homework.' => [
        'ar' => 'فشل في إرسال الواجب.',
    ],
    'Featured' => [
        'ar' => 'مميز',
    ],
    'Feedback:' => [
        'ar' => 'ملاحظات:',
    ],
    'Female' => [
        'ar' => 'أنثى',
    ],
    'File Available for Download' => [
        'ar' => 'ملف متاح للتنزيل',
    ],
    'Phone Support' => [
        'en' => 'Phone Support',
        'ar' => 'الدعم عبر الهاتف',
    ],
    'Email Address' => [
        'en' => 'Email Address',
        'ar' => 'عنوان البريد الإلكتروني',
    ],
    'Office Address' => [
        'en' => 'Office Address',
        'ar' => 'عنوان المكتب',
    ],
    'Find the perfect course for your learning journey' => [
        'ar' => 'اعثر على الدورة المناسبة لرحلتك التعليمية',
    ],
    'Finish Quiz' => [
        'ar' => 'إنهاء الاختبار',
    ],
    'Free' => [
        'ar' => 'مجاني',
    ],
    'Follow Us' => [
        'en' => 'Follow Us',
        'ar' => 'تابعنا',
    ],
    'Full Name' => [
        'ar' => 'الاسم الكامل',
    ],
    'Full name' => [
        'ar' => 'الاسم الكامل',
    ],
    'Gender' => [
        'ar' => 'الجنس',
    ],
    'General' => [
        'ar' => 'عام',
    ],
    'Get Certificate' => [
        'ar' => 'احصل على شهادة',
    ],
    'Download Certificate' => [
        'en' => 'Download Certificate',
        'ar' => 'تحميل الشهادة',
    ],
    'Course Completed!' => [
        'en' => 'Course Completed!',
        'ar' => 'تم إكمال الدورة!',
    ],
    'You have completed this course and earned a certificate.' => [
        'en' => 'You have completed this course and earned a certificate.',
        'ar' => 'لقد أكملت هذه الدورة وحصلت على شهادة.',
    ],
    'Get your certificate by entering your name.' => [
        'en' => 'Get your certificate by entering your name.',
        'ar' => 'احصل على شهادتك بإدخال اسمك.',
    ],
    'Request Certificate' => [
        'en' => 'Request Certificate',
        'ar' => 'طلب الشهادة',
    ],
    'Generate Certificate' => [
        'en' => 'Generate Certificate',
        'ar' => 'إنشاء الشهادة',
    ],
    'Name on Certificate' => [
        'en' => 'Name on Certificate',
        'ar' => 'الاسم على الشهادة',
    ],
    'Please enter the name you would like to appear on your certificate. This name will be printed exactly as you enter it.' => [
        'en' => 'Please enter the name you would like to appear on your certificate. This name will be printed exactly as you enter it.',
        'ar' => 'يرجى إدخال الاسم الذي تريد أن يظهر على شهادتك. سيتم طباعة هذا الاسم بالضبط كما تدخله.',
    ],
    'Enter your full name' => [
        'en' => 'Enter your full name',
        'ar' => 'أدخل اسمك الكامل',
    ],
    'This name will appear on your certificate. Make sure it is spelled correctly.' => [
        'en' => 'This name will appear on your certificate. Make sure it is spelled correctly.',
        'ar' => 'سيظهر هذا الاسم على شهادتك. تأكد من أنه مكتوب بشكل صحيح.',
    ],
    'Back to Course' => [
        'en' => 'Back to Course',
        'ar' => 'العودة إلى الدورة',
    ],
    'You have successfully completed the course' => [
        'en' => 'You have successfully completed the course',
        'ar' => 'لقد أكملت الدورة بنجاح',
    ],
    'Get Notified' => [
        'ar' => 'تلقي إشعارات',
    ],
    'Graded' => [
        'ar' => 'مُقيّم',
    ],
    'Homework' => [
        'ar' => 'واجب منزلي',
    ],
    'Homework Assignments' => [
        'ar' => 'واجبات منزلية',
    ],
    'Homework Details' => [
        'ar' => 'تفاصيل الواجب',
    ],
    'Homework Title' => [
        'ar' => 'عنوان الواجب',
    ],
    'Homework assignments will be posted here by your instructor.' => [
        'ar' => 'سيتم نشر الواجبات هنا من قبل مدربك.',
    ],
    'Homework submitted successfully!' => [
        'ar' => 'تم إرسال الواجب بنجاح!',
    ],
    'How can' => [
        'ar' => 'كيف يمكن',
    ],
    'IT & Software' => [
        'ar' => 'تكنولوجيا المعلومات والبرمجيات',
    ],
    'Instructor' => [
        'ar' => 'المدرب',
    ],
    'Instructor Profile' => [
        'ar' => 'ملف المدرب',
    ],
    'Instructors' => [
        'ar' => 'المدربون',
    ],
    'It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout.' => [
        'ar' => 'من الحقائق الثابتة أن القارئ سيتشتت بسبب محتوى الصفحة القابل للقراءة عند النظر إلى تصميمها.',
    ],
    'Join Class' => [
        'ar' => 'انضم إلى الفصل',
    ],
    'Join Live Class' => [
        'ar' => 'انضم إلى المحاضرة المباشرة',
    ],
    'Join Meeting' => [
        'ar' => 'انضم إلى الاجتماع',
    ],
    'Join our trading community! Fill out the form below to register as a trader.' => [
        'ar' => 'انضم إلى مجتمعنا التجاري! عبئ النموذج أدناه للتسجيل كتاجر.',
    ],
    'Just Added' => [
        'ar' => 'أضيفت حديثاً',
    ],
    'Lecture completed automatically!' => [
        'ar' => 'تم إكمال المحاضرة تلقائياً!',
    ],
    'LinkedIn' => [
        'ar' => 'لينكدإن',
    ],
    'LinkedIn profile URL' => [
        'ar' => 'رابط ملف لينكدإن',
    ],
    'List your certificates and qualifications' => [
        'ar' => 'اذكر شهاداتك ومؤهلاتك',
    ],
    'Live Class' => [
        'ar' => 'محاضرة مباشرة',
    ],
    'Live Classes' => [
        'ar' => 'حصص مباشرة',
    ],
    'Live Meeting Courses' => [
        'ar' => 'دورات الاجتماعات المباشرة',
    ],
    'Live Sessions' => [
        'ar' => 'جلسات مباشرة',
    ],
    'Live classes will be scheduled and appear here.' => [
        'ar' => 'سيتم جدولة الحصص المباشرة وستظهر هنا.',
    ],
    'Male' => [
        'ar' => 'ذكر',
    ],
    'Mark Selected as Complete' => [
        'ar' => 'تعيين المحدد كمكتمل',
    ],
    'Mark Selected as Incomplete' => [
        'ar' => 'تعيين المحدد كغير مكتمل',
    ],
    'Max Participants' => [
        'ar' => 'الحد الأقصى للمشاركين',
    ],
    'Meetings Requirements' => [
        'ar' => 'متطلبات الاجتماعات',
    ],
    'Minimum 3 characters required' => [
        'ar' => 'مطلوب 3 أحرف على الأقل',
    ],
    'N/A' => [
        'ar' => 'غير متاح',
    ],
    'Next' => [
        'ar' => 'التالي',
    ],
    'No Homework Assignments' => [
        'ar' => 'لا توجد واجبات منزلية',
    ],
    'No Live Classes Scheduled' => [
        'ar' => 'لا توجد حصص مباشرة مجدولة',
    ],
    'No answers yet. Be the first to answer this question!' => [
        'ar' => 'لا توجد إجابات بعد. كن أول من يجيب على هذا السؤال!',
    ],
    'No courses available in this category yet.' => [
        'ar' => 'لا توجد دورات متاحة في هذه الفئة بعد.',
    ],
    'No courses yet' => [
        'ar' => 'لا توجد دورات بعد',
    ],
    'No due date' => [
        'ar' => 'لا يوجد تاريخ استحقاق',
    ],
    'No homework assignments found.' => [
        'ar' => 'لم يتم العثور على واجبات منزلية.',
    ],
    'No questions yet' => [
        'ar' => 'لا توجد أسئلة بعد',
    ],
    'No Quizzes Yet' => [
        'ar' => 'لا توجد اختبارات بعد',
    ],
    'Quizzes for this course will appear here when added by your instructor.' => [
        'ar' => 'ستظهر اختبارات هذه الدورة هنا عند إضافتها من قبل المدرب.',
    ],
    'No ratings yet' => [
        'ar' => 'لا توجد تقييمات بعد',
    ],
    'No video available for this lecture' => [
        'ar' => 'لا يوجد فيديو لهذه المحاضرة',
    ],
    'Not Completed' => [
        'ar' => 'غير مكتمل',
    ],
    'Not Found' => [
        'ar' => 'غير موجود',
    ],
    'Not Started' => [
        'ar' => 'لم يبدأ',
    ],
    'Notes (Optional)' => [
        'ar' => 'ملاحظات (اختياري)',
    ],
    'Objective' => [
        'ar' => 'الهدف',
    ],
    'Only instructors and administrators can answer questions' => [
        'ar' => 'يمكن للمدربين والمسؤولين فقط الإجابة على الأسئلة',
    ],
    'Other' => [
        'ar' => 'أخرى',
    ],
    'Our Blog' => [
        'en' => 'Our Blog',
        'ar' => 'مدونتنا',
    ],
    'Our Courses' => [
        'ar' => 'دوراتنا',
    ],
    'Our Partners' => [
        'ar' => 'شركاؤنا',
    ],
    'Overview' => [
        'ar' => 'نظرة عامة',
    ],
    'Pending Approval' => [
        'ar' => 'قيد الموافقة',
    ],
    'Per Question Mark' => [
        'ar' => 'الدرجة لكل سؤال',
    ],
    'Percentage' => [
        'ar' => 'النسبة المئوية',
    ],
    'Phone' => [
        'ar' => 'الهاتف',
    ],
    'Phone number' => [
        'ar' => 'رقم الهاتف',
    ],
    'Photography' => [
        'ar' => 'التصوير',
    ],
    'Playing:' => [
        'ar' => 'جارٍ التشغيل:',
    ],
    'Please enter a question title.' => [
        'ar' => 'يرجى إدخال عنوان للسؤال.',
    ],
    'Please enter your answer' => [
        'ar' => 'يرجى إدخال إجابتك',
    ],
    'Please enter your answer.' => [
        'ar' => 'يرجى إدخال إجابتك.',
    ],
    'Please fill in all required fields' => [
        'ar' => 'يرجى تعبئة جميع الحقول المطلوبة',
    ],
    'Please fill in all required fields.' => [
        'ar' => 'يرجى تعبئة جميع الحقول المطلوبة.',
    ],
    'Please provide details about your report...' => [
        'ar' => 'يرجى تقديم تفاصيل حول بلاغك...',
    ],
    'Please select a question type' => [
        'ar' => 'يرجى اختيار نوع السؤال',
    ],
    'Points' => [
        'ar' => 'النقاط',
    ],
    'Previous' => [
        'ar' => 'السابق',
    ],
    'Primary Language' => [
        'ar' => 'اللغة الأساسية',
    ],
    'Primary language' => [
        'ar' => 'اللغة الأساسية',
    ],
    'Provide more details about your question...' => [
        'ar' => 'قدم المزيد من التفاصيل حول سؤالك...',
    ],
    'Q & A' => [
        'ar' => 'الأسئلة والأجوبة',
    ],
    'Question Details' => [
        'ar' => 'تفاصيل السؤال',
    ],
    'Question ID not found' => [
        'ar' => 'معرّف السؤال غير موجود',
    ],
    'Question Title' => [
        'ar' => 'عنوان السؤال',
    ],
    'Question Type' => [
        'ar' => 'نوع السؤال',
    ],
    'Question submitted successfully!' => [
        'ar' => 'تم إرسال السؤال بنجاح!',
    ],
    'Question title is required' => [
        'ar' => 'عنوان السؤال مطلوب',
    ],
    'Question title must be at least 3 characters' => [
        'ar' => 'يجب أن يكون عنوان السؤال 3 أحرف على الأقل',
    ],
    'Question:' => [
        'ar' => 'السؤال:',
    ],
    'Questions & Answers' => [
        'ar' => 'الأسئلة والأجوبة',
    ],
    'Quiz' => [
        'ar' => 'اختبار',
    ],
    'Quiz Completed!' => [
        'ar' => 'تم إكمال الاختبار!',
    ],
    'Quiz Price' => [
        'ar' => 'سعر الاختبار',
    ],
    'Quiz Timer' => [
        'ar' => 'مؤقت الاختبار',
    ],
    'Rating' => [
        'ar' => 'التقييم',
    ],
    'Reviews' => [
        'en' => 'Reviews',
        'ar' => 'التقييمات',
    ],
    'Read Less' => [
        'ar' => 'قراءة أقل',
    ],
    'Read More' => [
        'ar' => 'اقرأ المزيد',
    ],
    'Reason for joining' => [
        'ar' => 'سبب الانضمام',
    ],
    'Recent Activity' => [
        'ar' => 'النشاط الأخير',
    ],
    'Recent Courses' => [
        'ar' => 'الدورات الحديثة',
    ],
    'Recent Homework Assignments' => [
        'ar' => 'الواجبات المنزلية الحديثة',
    ],
    'Recent Questions' => [
        'ar' => 'الأسئلة الحديثة',
    ],
    'Recently' => [
        'ar' => 'مؤخراً',
    ],
    'Register as Trader' => [
        'ar' => 'سجل كتاجر',
    ],
    'Registered' => [
        'ar' => 'مسجل',
    ],
    'Registering...' => [
        'ar' => 'جاري التسجيل...',
    ],
    'Sending...' => [
        'en' => 'Sending...',
        'ar' => 'جاري الإرسال...',
    ],
    'Report Content' => [
        'ar' => 'الإبلاغ عن المحتوى',
    ],
    'Report Details' => [
        'ar' => 'تفاصيل البلاغ',
    ],
    'Report Question' => [
        'ar' => 'الإبلاغ عن السؤال',
    ],
    'Report Title' => [
        'ar' => 'عنوان البلاغ',
    ],
    'Report submitted successfully!' => [
        'ar' => 'تم إرسال البلاغ بنجاح!',
    ],
    'Sample Course' => [
        'ar' => 'دورة تجريبية',
    ],
    'Schedule & Deadlines' => [
        'ar' => 'الجدول والمواعيد النهائية',
    ],
    'Schedule Class' => [
        'ar' => 'جدولة فصل',
    ],
    'Schedule Live Class' => [
        'ar' => 'جدولة محاضرة مباشرة',
    ],
    'Score' => [
        'ar' => 'النتيجة',
    ],
    'Secondary Language' => [
        'ar' => 'اللغة الثانوية',
    ],
    'Secondary language' => [
        'ar' => 'اللغة الثانوية',
    ],
    'Select' => [
        'ar' => 'اختر',
    ],
    'Select question type' => [
        'ar' => 'اختر نوع السؤال',
    ],
    'Special Offers' => [
        'ar' => 'عروض خاصة',
    ],
    'Start Quiz' => [
        'ar' => 'ابدأ الاختبار',
    ],
    'Status' => [
        'ar' => 'الحالة',
    ],
    'Students' => [
        'ar' => 'الطلاب',
    ],
    'Student Inquiry Form' => [
        'en' => 'Student Inquiry Form',
        'ar' => 'نموذج استفسار الطالب',
    ],
    'Subject' => [
        'en' => 'Subject',
        'ar' => 'الموضوع',
    ],
    'Submit' => [
        'ar' => 'إرسال',
    ],
    'Submit Answer' => [
        'ar' => 'إرسال الإجابة',
    ],
    'Submit Assignment' => [
        'ar' => 'إرسال الواجب',
    ],
    'Submit Homework' => [
        'ar' => 'إرسال الواجب المنزلي',
    ],
    'Submit Question' => [
        'ar' => 'إرسال السؤال',
    ],
    'Submit Report' => [
        'ar' => 'إرسال البلاغ',
    ],
    'Submitted' => [
        'ar' => 'تم الإرسال',
    ],
    'Submitting...' => [
        'ar' => 'جاري الإرسال...',
    ],
    'Subscription Bundles' => [
        'ar' => 'باقات الاشتراك',
    ],
    'TBD' => [
        'en' => 'To be determined',
        'ar' => 'سيتم تحديده لاحقاً',
    ],
    'Technical' => [
        'ar' => 'تقني',
    ],
    'Test your knowledge of' => [
        'ar' => 'اختبر معرفتك في',
    ],
    'Test your knowledge of digital marketing fundamentals including SEO, social media marketing, content marketing, and online advertising strategies.' => [
        'ar' => 'اختبر معرفتك بأساسيات التسويق الرقمي بما في ذلك تحسين محركات البحث والتسويق عبر وسائل التواصل الاجتماعي وتسويق المحتوى واستراتيجيات الإعلان عبر الإنترنت.',
    ],
    'Testimonial' => [
        'ar' => 'شهادة عملاء',
    ],
    'Thank you for your registration! We will contact you soon.' => [
        'ar' => 'شكراً لتسجيلك! سنتواصل معك قريباً.',
    ],
    'Your message has been sent successfully!' => [
        'en' => 'Your message has been sent successfully!',
        'ar' => 'تم إرسال رسالتك بنجاح!',
    ],
    'This content is available as a downloadable file.' => [
        'ar' => 'هذا المحتوى متاح كملف قابل للتنزيل.',
    ],
    'This is a sample course description. Add some featured courses to see them here.' => [
        'ar' => 'هذا وصف دورة تجريبية. أضف بعض الدورات المميزة لتظهر هنا.',
    ],
    'This is an all-encompassing guide for making an independent feature le...' => [
        'ar' => 'هذا دليل شامل لصناعة فيلم روائي مستقل...',
    ],
    'This question type does not have multiple choice options.' => [
        'ar' => 'هذا النوع من الأسئلة لا يحتوي على خيارات متعددة.',
    ],
    'Time' => [
        'ar' => 'الوقت',
    ],
    'Time Taken' => [
        'ar' => 'الوقت المستغرق',
    ],
    'Title' => [
        'ar' => 'العنوان',
    ],
    'Top Discounted Courses' => [
        'ar' => 'أفضل الدورات المخفضة',
    ],
    'Total Marks' => [
        'ar' => 'إجمالي الدرجات',
    ],
    'Total Questions' => [
        'ar' => 'إجمالي الأسئلة',
    ],
    'Trader Registration' => [
        'ar' => 'تسجيل التاجر',
    ],
    'Trading Community' => [
        'ar' => 'مجتمع التداول',
    ],
    'Trading Experience' => [
        'ar' => 'خبرة التداول',
    ],
    'Training Experience' => [
        'ar' => 'خبرة التدريب',
    ],
    'Trending' => [
        'ar' => 'الرائج',
    ],
    'Trusted By Leading Organizations' => [
        'ar' => 'موثوق من قبل المؤسسات الرائدة',
    ],
    'Type' => [
        'ar' => 'النوع',
    ],
    'Upload Assignment File' => [
        'ar' => 'تحميل ملف الواجب',
    ],
    'Upload File' => [
        'ar' => 'تحميل ملف',
    ],
    'User' => [
        'ar' => 'المستخدم',
    ],
    'View' => [
        'ar' => 'عرض',
    ],
    'View All' => [
        'ar' => 'عرض الكل',
    ],
    'View All Categories' => [
        'ar' => 'عرض جميع الفئات',
    ],
    'View Course' => [
        'ar' => 'عرض الدورة',
    ],
    'View Detailed Results' => [
        'ar' => 'عرض النتائج التفصيلية',
    ],
    'View Details' => [
        'ar' => 'عرض التفاصيل',
    ],
    'View Profile' => [
        'ar' => 'عرض الملف الشخصي',
    ],
    'We partner with industry leaders to provide you with the best learning experience' => [
        'ar' => 'نحن نتعاون مع رواد الصناعة لنقدم لك أفضل تجربة تعلم',
    ],
    'Website' => [
        'ar' => 'الموقع الإلكتروني',
    ],
    'Website URL' => [
        'ar' => 'رابط الموقع الإلكتروني',
    ],
    'What Our Clients Says' => [
        'ar' => 'ماذا يقول عملاؤنا',
    ],
    'What are' => [
        'ar' => 'ما هي',
    ],
    'learn' => [
        'en' => 'Learn',
        'ar' => 'التعلم',
    ],
    'price' => [
        'en' => 'Price',
        'ar' => 'السعر',
    ],
    'value' => [
        'en' => 'Value',
        'ar' => 'القيمة',
    ],
    'WhatsApp' => [
        'ar' => 'واتساب',
    ],
    'WhatsApp number' => [
        'ar' => 'رقم الواتساب',
    ],
    'When are you available for meetings?' => [
        'ar' => 'متى تكون متاحاً للاجتماعات؟',
    ],
    'Why do you want to join this live class?' => [
        'ar' => 'لماذا تريد الانضمام إلى هذا الفصل المباشر؟',
    ],
    'Write your answer here...' => [
        'ar' => 'اكتب إجابتك هنا...',
    ],
    'You are registered for this class!' => [
        'ar' => 'أنت مسجل في هذا الفصل!',
    ],
    'You have already submitted this assignment.' => [
        'ar' => 'لقد أرسلت هذا الواجب بالفعل.',
    ],
    'You have joined the live class!' => [
        'ar' => 'لقد انضممت إلى الفصل المباشر!',
    ],
    'You have successfully completed the quiz.' => [
        'ar' => 'لقد أكملت الاختبار بنجاح.',
    ],
    'You will be notified when a homework assignment is posted.' => [
        'ar' => 'سيتم إخطارك عند نشر واجب منزلي.',
    ],
    'You will be notified when a live class is scheduled.' => [
        'ar' => 'سيتم إخطارك عند جدولة محاضرة مباشرة.',
    ],
    'Your Answer' => [
        'ar' => 'إجابتك',
    ],
    'Frequently Asked Questions' => [
        'en' => 'Frequently Asked Questions',
        'ar' => 'الأسئلة الشائعة',
    ],
    'No bio available' => [
        'en' => 'No bio available',
        'ar' => 'لا توجد نبذة متاحة',
    ],
    'No instructor assigned' => [
        'en' => 'No instructor assigned',
        'ar' => 'لم يتم تعيين مدرب',
    ],
    'Student Feedback' => [
        'en' => 'Student Feedback',
        'ar' => 'آراء الطلاب',
    ],
    'Course Rating' => [
        'en' => 'Course Rating',
        'ar' => 'تقييم الدورة',
    ],
    'Write review:' => [
        'en' => 'Write review:',
        'ar' => 'اكتب مراجعتك:',
    ],
    'Course Features' => [
        'en' => 'Course Features',
        'ar' => 'مميزات الدورة',
    ],
    'Course Preview' => [
        'en' => 'Course Preview',
        'ar' => 'معاينة الدورة',
    ],
    'Course Preview Video' => [
        'en' => 'Course Preview Video',
        'ar' => 'فيديو معاينة الدورة',
    ],
    'Your Message' => [
        'en' => 'Your Message',
        'ar' => 'رسالتك',
    ],
    'Your Email' => [
        'ar' => 'بريدك الإلكتروني',
    ],
    'Your Grade:' => [
        'ar' => 'درجتك:',
    ],
    'Your browser does not support the audio element.' => [
        'ar' => 'متصفحك لا يدعم عنصر الصوت.',
    ],
    'Your browser does not support the video tag.' => [
        'ar' => 'متصفحك لا يدعم وسم الفيديو.',
    ],
    'about_us' => [
        'en' => 'About Us',
        'ar' => 'من نحن',
    ],
    'academy_policy' => [
        'en' => 'Academy Policy',
        'ar' => 'سياسة الأكاديمية',
    ],
    'access_courses_immediately' => [
        'en' => 'Access courses immediately',
        'ar' => 'يمكنك الوصول إلى الدورات فوراً',
    ],
    'address' => [
        'en' => 'Address',
        'ar' => 'العنوان',
    ],
    'all_categories' => [
        'en' => 'All Categories',
        'ar' => 'جميع الفئات',
    ],
    'all_notifications' => [
        'en' => 'All Notifications',
        'ar' => 'جميع الإشعارات',
    ],
    'answers' => [
        'en' => 'Answers',
        'ar' => 'الإجابات',
    ],
    'apply' => [
        'en' => 'Apply',
        'ar' => 'تطبيق',
    ],
    'apply_coupon' => [
        'en' => 'Apply Coupon',
        'ar' => 'تطبيق القسيمة',
    ],
    'back_to_checkout' => [
        'en' => 'Back to Checkout',
        'ar' => 'العودة إلى الدفع',
    ],
    'back_to_home' => [
        'en' => 'Back to Home',
        'ar' => 'العودة إلى الصفحة الرئيسية',
    ],
    'benefit' => [
        'en' => 'Benefit',
        'ar' => 'الفائدة',
    ],
    'billing_information' => [
        'en' => 'Billing Information',
        'ar' => 'معلومات الفواتير',
    ],
    'blog' => [
        'en' => 'Blog',
        'ar' => 'المدونة',
    ],
    'browse_all_categories' => [
        'en' => 'Browse All Categories',
        'ar' => 'تصفح كل الفئات',
    ],
    'browse_all_courses' => [
        'en' => 'Browse All Courses',
        'ar' => 'تصفح جميع الدورات',
    ],
    'browse_categories_description' => [
        'en' => 'Explore our range of categories and choose what suits you',
        'ar' => 'استكشف فئاتنا المتنوعة واختر ما يناسبك',
    ],
    'browse_courses' => [
        'en' => 'Browse Courses',
        'ar' => 'تصفح الدورات',
    ],
    'browse_more_courses' => [
        'en' => 'Browse More Courses',
        'ar' => 'تصفح المزيد من الدورات',
    ],
    'popular_categories' => [
        'en' => 'Popular Categories',
        'ar' => 'الفئات الشائعة',
    ],
    'card_holder_name' => [
        'en' => 'Cardholder Name',
        'ar' => 'اسم حامل البطاقة',
    ],
    'card_holder_placeholder' => [
        'en' => 'Enter cardholder name',
        'ar' => 'أدخل اسم حامل البطاقة',
    ],
    'card_number' => [
        'en' => 'Card Number',
        'ar' => 'رقم البطاقة',
    ],
    'cart_description' => [
        'en' => 'Review the items in your cart before proceeding to checkout.',
        'ar' => 'راجع العناصر في سلة التسوق الخاصة بك قبل المتابعة.',
    ],
    'cart_items' => [
        'en' => 'Cart Items',
        'ar' => 'عناصر السلة',
    ],
    'continue_shopping' => [
        'en' => 'Continue Shopping',
        'ar' => 'متابعة التسوق',
    ],
    'categories' => [
        'en' => 'Categories',
        'ar' => 'الفئات',
    ],
    'category_detail' => [
        'en' => 'Category Detail',
        'ar' => 'تفاصيل الفئة',
    ],
    'checkout' => [
        'en' => 'Checkout',
        'ar' => 'إتمام الشراء',
    ],
    'cart' => [
        'en' => 'Cart',
        'ar' => 'السلة',
    ],
    'complete' => [
        'en' => 'Complete',
        'ar' => 'إكمال',
    ],
    'city' => [
        'en' => 'City',
        'ar' => 'المدينة',
    ],
    'clear_filters' => [
        'en' => 'Clear Filters',
        'ar' => 'إزالة عوامل التصفية',
    ],
    'coming_soon' => [
        'en' => 'Coming Soon',
        'ar' => 'قريباً',
    ],
    'coming_soon.subscription_error' => [
        'en' => 'An error occurred during subscription.',
        'ar' => 'حدث خطأ أثناء الاشتراك.',
    ],
    'complete_purchase' => [
        'en' => 'Complete Purchase',
        'ar' => 'إكمال الشراء',
    ],
    'complete_your_payment' => [
        'en' => 'Complete your payment',
        'ar' => 'أكمل عملية الدفع',
    ],
    'complete_your_purchase' => [
        'en' => 'Complete your purchase',
        'ar' => 'أكمل عملية الشراء',
    ],
    'enter_billing_details' => [
        'en' => 'Enter your billing details',
        'ar' => 'أدخل تفاصيل الفواتير الخاصة بك',
    ],
    'select_payment_method' => [
        'en' => 'Select your preferred payment method',
        'ar' => 'اختر طريقة الدفع المفضلة لديك',
    ],
    'completed' => [
        'en' => 'Completed',
        'ar' => 'مكتمل',
    ],
    'contact' => [
        'en' => 'Contact',
        'ar' => 'اتصل بنا',
    ],
    'contact_us' => [
        'en' => 'Contact Us',
        'ar' => 'تواصل معنا',
    ],
    'country' => [
        'en' => 'Country',
        'ar' => 'الدولة',
    ],
    'coupon_description' => [
        'en' => 'Enter a coupon code to unlock a discount on your order.',
        'ar' => 'أدخل رمز القسيمة للحصول على خصم على طلبك.',
    ],
    'course' => [
        'en' => 'Course',
        'ar' => 'دورة',
    ],
    'courses' => [
        'en' => 'Courses',
        'ar' => 'الدورات',
    ],
    'courses_found' => [
        'en' => 'Courses Found',
        'ar' => 'الدورات التي تم العثور عليها',
    ],
    'credit_card' => [
        'en' => 'Credit Card',
        'ar' => 'بطاقة ائتمان',
    ],
    'credit_card_description' => [
        'en' => 'Pay securely with your credit card.',
        'ar' => 'ادفع بأمان باستخدام بطاقتك الائتمانية.',
    ],
    'credit_card_payment' => [
        'en' => 'Credit Card Payment',
        'ar' => 'دفع ببطاقة الائتمان',
    ],
    'cvv' => [
        'en' => 'CVV',
        'ar' => 'رمز التحقق CVV',
    ],
    'delete_notification_confirm' => [
        'en' => 'Are you sure you want to delete this notification?',
        'ar' => 'هل أنت متأكد أنك تريد حذف هذا الإشعار؟',
    ],
    'discount' => [
        'en' => 'Discount',
        'ar' => 'الخصم',
    ],
    'duration' => [
        'en' => 'Duration',
        'ar' => 'المدة',
    ],
    'email' => [
        'en' => 'Email',
        'ar' => 'البريد الإلكتروني',
    ],
    'empty_cart' => [
        'en' => 'Your cart is empty',
        'ar' => 'سلة التسوق فارغة',
    ],
    'empty_cart_message' => [
        'en' => 'You have not added any courses yet.',
        'ar' => 'لم تقم بإضافة أي دورات بعد.',
    ],
    'empty_wishlist' => [
        'en' => 'Your wishlist is empty',
        'ar' => 'قائمة الرغبات فارغة',
    ],
    'empty_wishlist_message' => [
        'en' => 'You have not added any courses to your wishlist yet.',
        'ar' => 'لم تضف أي دورات إلى قائمة الرغبات بعد.',
    ],
    'enroll_now' => [
        'en' => 'Enroll Now',
        'ar' => 'سجل الآن',
    ],
    'enrolled' => [
        'en' => 'Enrolled',
        'ar' => 'مسجل',
    ],
    'Enrolled successfully' => [
        'en' => 'Enrolled successfully',
        'ar' => 'تم التسجيل بنجاح',
    ],
    'enrolled_courses' => [
        'en' => 'Enrolled Courses',
        'ar' => 'الدورات المسجلة',
    ],
    'enter_coupon_code' => [
        'en' => 'Enter coupon code',
        'ar' => 'أدخل رمز القسيمة',
    ],
    'expiry_date' => [
        'en' => 'Expiry Date',
        'ar' => 'تاريخ الانتهاء',
    ],
    'explore' => [
        'en' => 'Explore',
        'ar' => 'استكشف',
    ],
    'explore_categories' => [
        'en' => 'Explore Categories',
        'ar' => 'استكشف الفئات',
    ],
    'featured' => [
        'en' => 'Featured',
        'ar' => 'مميز',
    ],
    'featured_categories' => [
        'en' => 'Featured Categories',
        'ar' => 'الفئات المميزة',
    ],
    'featured_categories_description' => [
        'en' => 'Explore our most popular course categories',
        'ar' => 'استكشف أكثر فئات الدورات شعبية لدينا',
    ],
    'featured_courses' => [
        'en' => 'Featured Courses',
        'ar' => 'دورات مميزة',
    ],
    'filters' => [
        'en' => 'Filters',
        'ar' => 'عوامل التصفية',
    ],
    'first_name' => [
        'en' => 'First Name',
        'ar' => 'الاسم الأول',
    ],
    'found_courses' => [
        'en' => 'Found Courses',
        'ar' => 'الدورات التي تم العثور عليها',
    ],
    'free' => [
        'en' => 'Free',
        'ar' => 'مجاني',
    ],
    'free_enrollment' => [
        'en' => 'Free Enrollment',
        'ar' => 'تسجيل مجاني',
    ],
    'free_enrollment_description' => [
        'en' => 'Enroll in this course at no cost.',
        'ar' => 'سجل في هذه الدورة دون أي تكلفة.',
    ],
    'fundamentals and concepts.' => [
        'ar' => 'الأساسيات والمفاهيم.',
    ],
    'go_home' => [
        'en' => 'Go Home',
        'ar' => 'اذهب إلى الصفحة الرئيسية',
    ],
    'go_to_course' => [
        'en' => 'Go to Course',
        'ar' => 'اذهب إلى الدورة',
    ],
    'go_to_dashboard' => [
        'en' => 'Go to Dashboard',
        'ar' => 'اذهب إلى لوحة التحكم',
    ],
    'highest_rated' => [
        'en' => 'Highest Rated',
        'ar' => 'الأعلى تقييماً',
    ],
    'home' => [
        'en' => 'Home',
        'ar' => 'الرئيسية',
    ],
    'instructor' => [
        'en' => 'Instructor',
        'ar' => 'المدرب',
    ],
    'item(s) marked as complete!' => [
        'en' => 'Item(s) marked as complete!',
        'ar' => 'تم تحديد العناصر كمكتملة!',
    ],
    'item(s) marked as incomplete!' => [
        'en' => 'Item(s) marked as incomplete!',
        'ar' => 'تم تحديد العناصر كغير مكتملة!',
    ],
    'language' => [
        'en' => 'Language',
        'ar' => 'اللغة',
    ],
    'last_name' => [
        'en' => 'Last Name',
        'ar' => 'اسم العائلة',
    ],
    'latest_posts' => [
        'en' => 'Latest Posts',
        'ar' => 'أحدث المقالات',
    ],
    'Latest Blog & News' => [
        'en' => 'Latest Blog & News',
        'ar' => 'أحدث المدونات والأخبار',
    ],
    'lectures' => [
        'en' => 'Lectures',
        'ar' => 'المحاضرات',
    ],
    'limit' => [
        'en' => 'Limit',
        'ar' => 'الحد',
    ],
    'login' => [
        'en' => 'Log In',
        'ar' => 'تسجيل الدخول',
    ],
    'logout' => [
        'en' => 'Log Out',
        'ar' => 'تسجيل الخروج',
    ],
    'Make An Request' => [
        'en' => 'Make An Request',
        'ar' => 'قدّم طلباً',
    ],
    'manage_your_notifications' => [
        'en' => 'Manage your notifications',
        'ar' => 'إدارة إشعاراتك',
    ],
    'mark_all_read' => [
        'en' => 'Mark All Read',
        'ar' => 'وضع علامة مقروء للجميع',
    ],
    'minutes' => [
        'en' => 'Minutes',
        'ar' => 'دقائق',
    ],
    'my_courses' => [
        'en' => 'My Courses',
        'ar' => 'دوراتي',
    ],
    'my_wishlist' => [
        'en' => 'My Wishlist',
        'ar' => 'قائمة رغباتي',
    ],
    'name' => [
        'en' => 'Name',
        'ar' => 'الاسم',
    ],
    'new' => [
        'en' => 'New',
        'ar' => 'جديد',
    ],
    'newest' => [
        'en' => 'Newest',
        'ar' => 'الأحدث',
    ],
    'no_blog_posts' => [
        'en' => 'No blog posts',
        'ar' => 'لا توجد مقالات مدونة',
    ],
    'no_categories_found' => [
        'en' => 'No categories found',
        'ar' => 'لم يتم العثور على فئات',
    ],
    'no_categories_message' => [
        'en' => 'No categories have been added yet.',
        'ar' => 'لم تتم إضافة أي فئات حتى الآن.',
    ],
    'no_category_found' => [
        'en' => 'No category found',
        'ar' => 'لم يتم العثور على فئة',
    ],
    'no_courses_found' => [
        'en' => 'No courses found',
        'ar' => 'لم يتم العثور على دورات',
    ],
    'no_courses_found_message' => [
        'en' => 'Try different filters or another search term.',
        'ar' => 'حاول استخدام مرشحات مختلفة أو كلمة بحث أخرى.',
    ],
    'no_courses_in_category' => [
        'en' => 'No courses in this category',
        'ar' => 'لا توجد دورات في هذه الفئة',
    ],
    'no_description_available' => [
        'en' => 'No description available',
        'ar' => 'لا يوجد وصف متاح',
    ],
    'no_notifications' => [
        'en' => 'No notifications',
        'ar' => 'لا توجد إشعارات',
    ],
    'no_notifications_message' => [
        'en' => 'You will see notifications here.',
        'ar' => 'ستظهر إشعاراتك هنا.',
    ],
    'notes' => [
        'en' => 'Notes',
        'ar' => 'ملاحظات',
    ],
    'notes_placeholder' => [
        'en' => 'Add your notes here',
        'ar' => 'أضف ملاحظاتك هنا',
    ],
    'notifications' => [
        'en' => 'Notifications',
        'ar' => 'الإشعارات',
    ],
    'notifications_found' => [
        'en' => 'Notifications found',
        'ar' => 'الإشعارات الموجودة',
    ],
    'order_date' => [
        'en' => 'Order Date',
        'ar' => 'تاريخ الطلب',
    ],
    'order_details' => [
        'en' => 'Order Details',
        'ar' => 'تفاصيل الطلب',
    ],
    'order_number' => [
        'en' => 'Order Number',
        'ar' => 'رقم الطلب',
    ],
    'order_successful' => [
        'en' => 'Order Successful',
        'ar' => 'تم الطلب بنجاح',
    ],
    'order_summary' => [
        'en' => 'Order Summary',
        'ar' => 'ملخص الطلب',
    ],
    'our_links' => [
        'en' => 'Our Links',
        'ar' => 'روابطنا',
    ],
    'paid' => [
        'en' => 'Paid',
        'ar' => 'مدفوع',
    ],
    'pay_now' => [
        'en' => 'Pay Now',
        'ar' => 'ادفع الآن',
    ],
    'payment' => [
        'en' => 'Payment',
        'ar' => 'الدفع',
    ],
    'payment_method' => [
        'en' => 'Payment Method',
        'ar' => 'طريقة الدفع',
    ],
    'payment_placeholder_message' => [
        'en' => 'The payment gateway will be integrated soon.',
        'ar' => 'سيتم دمج بوابة الدفع قريباً.',
    ],
    'phone' => [
        'en' => 'Phone',
        'ar' => 'الهاتف',
    ],
    'points' => [
        'en' => 'Points',
        'ar' => 'النقاط',
    ],
    'popular' => [
        'en' => 'Popular',
        'ar' => 'شائع',
    ],
    'postal_code' => [
        'en' => 'Postal Code',
        'ar' => 'الرمز البريدي',
    ],
    'price' => [
        'en' => 'Price',
        'ar' => 'السعر',
    ],
    'price_high_to_low' => [
        'en' => 'Price: High to Low',
        'ar' => 'السعر: من الأعلى إلى الأدنى',
    ],
    'price_low_to_high' => [
        'en' => 'Price: Low to High',
        'ar' => 'السعر: من الأدنى إلى الأعلى',
    ],
    'proceed_to_checkout' => [
        'en' => 'Proceed to Checkout',
        'ar' => 'المتابعة للدفع',
    ],
    'profile' => [
        'en' => 'Profile',
        'ar' => 'الملف الشخصي',
    ],
    'purchase_history' => [
        'en' => 'Purchase History',
        'ar' => 'سجل المشتريات',
    ],
    'rating' => [
        'en' => 'Rating',
        'ar' => 'التقييم',
    ],
    'receive_certificate_completion' => [
        'en' => 'Receive a certificate of completion',
        'ar' => 'احصل على شهادة إتمام',
    ],
    'register' => [
        'en' => 'Register',
        'ar' => 'تسجيل',
    ],
    'remove' => [
        'en' => 'Remove',
        'ar' => 'إزالة',
    ],
    'remove_from_cart_confirm' => [
        'en' => 'Remove this item from the cart?',
        'ar' => 'هل تريد إزالة هذا العنصر من السلة؟',
    ],
    'remove_from_wishlist_confirm' => [
        'en' => 'Remove this item from the wishlist?',
        'ar' => 'هل تريد إزالة هذا العنصر من قائمة الرغبات؟',
    ],
    'search_courses' => [
        'en' => 'Search Courses',
        'ar' => 'ابحث عن الدورات',
    ],
    'search_results' => [
        'en' => 'Search Results',
        'ar' => 'نتائج البحث',
    ],
    'search_results_for' => [
        'en' => 'Search results for',
        'ar' => 'نتائج البحث عن',
    ],
    'Search your courses...' => [
        'en' => 'Search your courses...',
        'ar' => 'ابحث في دوراتك...',
    ],
    'secure_checkout' => [
        'en' => 'Secure Checkout',
        'ar' => 'دفع آمن',
    ],
    'secure_payment_message' => [
        'en' => 'Your payments are secure and encrypted.',
        'ar' => 'دفعاتك آمنة ومشفرة.',
    ],
    'sale' => [
        'en' => 'Sale',
        'ar' => 'تخفيض',
    ],
    'off' => [
        'en' => 'off',
        'ar' => 'خصم',
    ],
    'select_experience' => [
        'en' => 'Select years of experience',
        'ar' => 'اختر سنوات الخبرة',
    ],
    'shopping_cart' => [
        'en' => 'Shopping Cart',
        'ar' => 'سلة التسوق',
    ],
    'items' => [
        'en' => 'Items',
        'ar' => 'العناصر',
    ],
    'certified' => [
        'en' => 'Certified',
        'ar' => 'معتمد',
    ],
    'lifetime_access' => [
        'en' => 'Lifetime Access',
        'ar' => 'وصول مدى الحياة',
    ],
    'mobile_access' => [
        'en' => 'Mobile Access',
        'ar' => 'وصول عبر الجوال',
    ],
    'sign_with_us' => [
        'en' => 'Sign With Us',
        'ar' => 'سجل معنا',
    ],
    'sort' => [
        'en' => 'Sort',
        'ar' => 'ترتيب',
    ],
    'stars' => [
        'en' => 'Stars',
        'ar' => 'نجوم',
    ],
    'start_learning_right_away' => [
        'en' => 'Start learning right away',
        'ar' => 'ابدأ التعلم فوراً',
    ],
    'state' => [
        'en' => 'State',
        'ar' => 'الولاية',
    ],
    'status' => [
        'en' => 'Status',
        'ar' => 'الحالة',
    ],
    'students' => [
        'en' => 'Students',
        'ar' => 'الطلاب',
    ],
    'lessons' => [
        'en' => 'Lessons',
        'ar' => 'دروس',
    ],
    'submit' => [
        'en' => 'Submit',
        'ar' => 'إرسال',
    ],
    'subscription_error' => [
        'en' => 'An error occurred during subscription.',
        'ar' => 'حدث خطأ أثناء الاشتراك.',
    ],
    'subtitle' => [
        'en' => 'Subtitle',
        'ar' => 'العنوان الفرعي',
    ],
    'subtotal' => [
        'en' => 'Subtotal',
        'ar' => 'الإجمالي الفرعي',
    ],
    'tax' => [
        'en' => 'Tax',
        'ar' => 'الضريبة',
    ],
    'processing' => [
        'en' => 'Processing...',
        'ar' => 'جارٍ المعالجة...',
    ],
    'terms_and_conditions' => [
        'en' => 'Terms & Conditions',
        'ar' => 'الشروط والأحكام',
    ],
    'please_select_payment_method' => [
        'en' => 'Please select a payment method',
        'ar' => 'يرجى اختيار طريقة الدفع',
    ],
    '30_day_money_back' => [
        'en' => '30-Day Money Back Guarantee',
        'ar' => 'ضمان استعادة الأموال خلال 30 يومًا',
    ],
    'satisfaction_guaranteed' => [
        'en' => 'Your satisfaction is guaranteed.',
        'ar' => 'رضاك مضمون.',
    ],
    'thank_you_for_your_purchase' => [
        'en' => 'Thank you for your purchase',
        'ar' => 'شكراً لشرائك',
    ],
    'title' => [
        'en' => 'Title',
        'ar' => 'العنوان',
    ],
    'total' => [
        'en' => 'Total',
        'ar' => 'الإجمالي',
    ],
    'total_amount' => [
        'en' => 'Total Amount',
        'ar' => 'إجمالي المبلغ',
    ],
    'track_progress_dashboard' => [
        'en' => 'Track your progress from the dashboard',
        'ar' => 'تابع تقدمك من لوحة التحكم',
    ],
    'view_all_notifications' => [
        'en' => 'View All Notifications',
        'ar' => 'عرض جميع الإشعارات',
    ],
    'view_course' => [
        'en' => 'View Course',
        'ar' => 'عرض الدورة',
    ],
    'view_courses' => [
        'en' => 'View Courses',
        'ar' => 'عرض الدورات',
    ],
    'view_details' => [
        'en' => 'View Details',
        'ar' => 'عرض التفاصيل',
    ],
    'what_happens_next' => [
        'en' => 'What happens next',
        'ar' => 'ماذا سيحدث بعد ذلك',
    ],
    'whatsapp_number' => [
        'en' => 'WhatsApp Number',
        'ar' => 'رقم الواتساب',
    ],
    'wishlist' => [
        'en' => 'Wishlist',
        'ar' => 'قائمة الرغبات',
    ],
    'wishlist_description' => [
        'en' => 'Save courses you plan to revisit later.',
        'ar' => 'احفظ الدورات التي ترغب في متابعتها لاحقاً.',
    ],
    'years' => [
        'en' => 'Years',
        'ar' => 'سنوات',
    ],
    'years_of_experience' => [
        'en' => 'Years of Experience',
        'ar' => 'سنوات الخبرة',
    ],
    'you_will_see_notifications_here' => [
        'en' => 'You will see notifications here',
        'ar' => 'ستظهر إشعاراتك هنا',
    ],
    'Account Information' => [
        'ar' => 'معلومات الحساب',
    ],
    'Account Status' => [
        'ar' => 'حالة الحساب',
    ],
    'Active' => [
        'ar' => 'نشط',
    ],
    'Age' => [
        'ar' => 'العمر',
    ],
    'All Status' => [
        'ar' => 'جميع الحالات',
    ],
    'Avg Progress' => [
        'ar' => 'متوسط التقدم',
    ],
    'Beginner' => [
        'ar' => 'مبتدئ',
    ],
    'Bio' => [
        'ar' => 'نبذة شخصية',
    ],
    'Browse Courses' => [
        'ar' => 'تصفح الدورات',
    ],
    'Change Password' => [
        'ar' => 'تغيير كلمة المرور',
    ],
    'characters' => [
        'ar' => 'حرف',
    ],
    'Confirm New Password' => [
        'ar' => 'تأكيد كلمة المرور الجديدة',
    ],
    'Continue' => [
        'ar' => 'متابعة',
    ],
    'Continue Learning' => [
        'ar' => 'تابع التعلم',
    ],
    'Course Name' => [
        'ar' => 'اسم الدورة',
    ],
    'Current Password' => [
        'ar' => 'كلمة المرور الحالية',
    ],
    'Date of Birth' => [
        'ar' => 'تاريخ الميلاد',
    ],
    'Edit Profile' => [
        'ar' => 'تعديل الملف الشخصي',
    ],
    'Email Verified' => [
        'ar' => 'البريد الإلكتروني موثق',
    ],
    'Enrolled' => [
        'ar' => 'تاريخ الالتحاق',
    ],
    'Enter your phone number' => [
        'ar' => 'أدخل رقم هاتفك',
    ],
    'Find More Courses' => [
        'ar' => 'اعثر على المزيد من الدورات',
    ],
    'Filter' => [
        'ar' => 'تصفية',
    ],
    'In Progress' => [
        'ar' => 'قيد التقدم',
    ],
    'Last Updated' => [
        'ar' => 'آخر تحديث',
    ],
    'Latest First' => [
        'ar' => 'الأحدث أولاً',
    ],
    'lectures completed' => [
        'ar' => 'محاضرات مكتملة',
    ],
    'Level' => [
        'ar' => 'المستوى',
    ],
    'Member Since' => [
        'ar' => 'عضو منذ',
    ],
    'My Courses' => [
        'ar' => 'دوراتي',
    ],
    'New Password' => [
        'ar' => 'كلمة مرور جديدة',
    ],
    'No Courses Found' => [
        'ar' => 'لم يتم العثور على دورات',
    ],
    'No courses match your current filters. Try adjusting your search criteria.' => [
        'ar' => 'لا توجد دورات تطابق عوامل التصفية الحالية. حاول تعديل معايير البحث.',
    ],
    'Not Verified' => [
        'ar' => 'غير موثق',
    ],
    'Oldest First' => [
        'ar' => 'الأقدم أولاً',
    ],
    'Phone Number' => [
        'ar' => 'رقم الهاتف',
    ],
    'Please fix the following errors:' => [
        'ar' => 'يرجى تصحيح الأخطاء التالية:',
    ],
    'Profile Information' => [
        'ar' => 'معلومات الملف الشخصي',
    ],
    'Profile Picture' => [
        'ar' => 'صورة الملف الشخصي',
    ],
    'Progress' => [
        'ar' => 'التقدم',
    ],
    'Progress High' => [
        'ar' => 'أعلى نسبة تقدم',
    ],
    'Progress Low' => [
        'ar' => 'أقل نسبة تقدم',
    ],
    'Select Country' => [
        'ar' => 'اختر الدولة',
    ],
    'Select Gender' => [
        'ar' => 'اختر الجنس',
    ],
    'Tell us about yourself, your interests, or what you\'re learning...' => [
        'ar' => 'أخبرنا عن نفسك، واهتماماتك، أو ما تتعلمه...',
    ],
    'Total Courses' => [
        'ar' => 'إجمالي الدورات',
    ],
    'Track your learning progress and continue your courses' => [
        'ar' => 'تابع تقدمك في التعلم واستمر في دوراتك',
    ],
    'Unknown Instructor' => [
        'ar' => 'مدرب غير معروف',
    ],
    'Uncategorized' => [
        'ar' => 'غير مصنف',
    ],
    'Update Profile' => [
        'ar' => 'تحديث الملف الشخصي',
    ],
    'Upload a profile picture (JPG, PNG, GIF - Max 2MB)' => [
        'ar' => 'حمّل صورة للملف الشخصي (JPG, PNG, GIF - بحد أقصى 2 ميجا بايت)',
    ],
    'Verified' => [
        'ar' => 'موثق',
    ],
    'You haven\'t enrolled in any courses yet. Start your learning journey today!' => [
        'ar' => 'لم تلتحق بأي دورة بعد. ابدأ رحلتك التعليمية اليوم!',
    ],
    'years old' => [
        'ar' => 'عاماً',
    ],
    'Course added to cart' => [
        'en' => 'Course added to cart',
        'ar' => 'تمت إضافة الدورة إلى السلة',
    ],
    'Course added to cart successfully.' => [
        'en' => 'Course added to cart successfully.',
        'ar' => 'تمت إضافة الدورة إلى السلة بنجاح.',
    ],
    'You can continue shopping or proceed to checkout to complete your purchase.' => [
        'en' => 'You can continue shopping or proceed to checkout to complete your purchase.',
        'ar' => 'يمكنك متابعة التسوق أو الانتقال إلى الدفع لإكمال عملية الشراء.',
    ],
    'Continue shopping' => [
        'en' => 'Continue shopping',
        'ar' => 'متابعة التسوق',
    ],
    'Go to checkout' => [
        'en' => 'Go to checkout',
        'ar' => 'الانتقال إلى الدفع',
    ],
    'Review submitted successfully!' => [
        'en' => 'Review submitted successfully!',
        'ar' => 'تم إرسال التقييم بنجاح!',
    ],
    'Please select a star rating before submitting your review.' => [
        'en' => 'Please select a star rating before submitting your review.',
        'ar' => 'يرجى اختيار تقييم بالنجوم قبل إرسال رأيك.',
    ],
    'Unable to submit review. Please try again.' => [
        'en' => 'Unable to submit review. Please try again.',
        'ar' => 'تعذر إرسال التقييم. يرجى المحاولة مرة أخرى.',
    ],
    'An unexpected error occurred. Please try again later.' => [
        'en' => 'An unexpected error occurred. Please try again later.',
        'ar' => 'حدث خطأ غير متوقع. يرجى المحاولة مرة أخرى لاحقاً.',
    ],
    'Unable to enroll in this course.' => [
        'en' => 'Unable to enroll in this course.',
        'ar' => 'تعذر التسجيل في هذه الدورة.',
    ],
    'Unable to add course to cart.' => [
        'en' => 'Unable to add course to cart.',
        'ar' => 'تعذر إضافة الدورة إلى السلة.',
    ],
    'Student' => [
        'en' => 'Student',
        'ar' => 'طالب',
    ],
    'View all reviews' => [
        'en' => 'View all reviews',
        'ar' => 'عرض جميع التقييمات',
    ],
    'grid' => [
        'en' => 'Grid',
        'ar' => 'شبكة',
    ],
    'list' => [
        'en' => 'List',
        'ar' => 'قائمة',
    ],
    'Related Courses' => [
        'en' => 'Related Courses',
        'ar' => 'دورات ذات صلة',
    ],
    'We Accept' => [
        'en' => 'We Accept',
        'ar' => 'نقبل',
    ],
    'Paid' => [
        'en' => 'Paid',
        'ar' => 'مدفوع',
    ],
    'Sort By' => [
        'en' => 'Sort By',
        'ar' => 'ترتيب حسب',
    ],
    'Latest' => [
        'en' => 'Latest',
        'ar' => 'الأحدث',
    ],
    'Most Popular' => [
        'en' => 'Most Popular',
        'ar' => 'الأكثر شعبية',
    ],
    'Highest Rated' => [
        'en' => 'Highest Rated',
        'ar' => 'الأعلى تقييماً',
    ],
    'Price: Low to High' => [
        'en' => 'Price: Low to High',
        'ar' => 'السعر: من الأقل إلى الأعلى',
    ],
    'Price: High to Low' => [
        'en' => 'Price: High to Low',
        'ar' => 'السعر: من الأعلى إلى الأقل',
    ],
    'View' => [
        'en' => 'View',
        'ar' => 'عرض',
    ],
    'Blog' => [
        'en' => 'Blog',
        'ar' => 'المدونة',
    ],
    'Categories' => [
        'en' => 'Categories',
        'ar' => 'التصنيفات',
    ],
    'All Categories' => [
        'en' => 'All Categories',
        'ar' => 'جميع التصنيفات',
    ],
    'Search Blogs' => [
        'en' => 'Search Blogs',
        'ar' => 'البحث في المدونة',
    ],
    'Search blogs...' => [
        'en' => 'Search blogs...',
        'ar' => 'البحث في المدونات...',
    ],
    'Filtered by:' => [
        'en' => 'Filtered by:',
        'ar' => 'تصفية حسب:',
    ],
    'Category:' => [
        'en' => 'Category:',
        'ar' => 'التصنيف:',
    ],
    'Clear All' => [
        'en' => 'Clear All',
        'ar' => 'مسح الكل',
    ],
    'Featured' => [
        'en' => 'Featured',
        'ar' => 'مميز',
    ],
    'views' => [
        'en' => 'views',
        'ar' => 'مشاهدات',
    ],
    'Read More' => [
        'en' => 'Read More',
        'ar' => 'اقرأ المزيد',
    ],
    'No blogs found' => [
        'en' => 'No blogs found',
        'ar' => 'لم يتم العثور على مدونات',
    ],
    'Recent Posts' => [
        'en' => 'Recent Posts',
        'ar' => 'المقالات الأخيرة',
    ],
    'Back to Blog' => [
        'en' => 'Back to Blog',
        'ar' => 'العودة إلى المدونة',
    ],
    'Share' => [
        'en' => 'Share',
        'ar' => 'مشاركة',
    ],
    'Related Articles' => [
        'en' => 'Related Articles',
        'ar' => 'مقالات ذات صلة',
    ],
    'No recent posts.' => [
        'en' => 'No recent posts.',
        'ar' => 'لا توجد مقالات حديثة.',
    ],
    'Check back later for new blog posts.' => [
        'en' => 'Check back later for new blog posts.',
        'ar' => 'عد لاحقاً للمزيد من المقالات.',
    ],
    'Summary' => [
        'en' => 'Summary',
        'ar' => 'ملخص',
    ],
    'Tags:' => [
        'en' => 'Tags:',
        'ar' => 'الوسوم:',
    ],
    'By' => [
        'en' => 'By',
        'ar' => 'بواسطة',
    ],
    'verify_email_title' => [
        'en' => 'Verify Your Email Address',
        'ar' => 'تأكيد عنوان بريدك الإلكتروني',
    ],
    'verify_email_message' => [
        'en' => 'Thanks for signing up! Before getting started, could you verify your email address by clicking on the link we just emailed to you? If you didn\'t receive the email, we will gladly send you another.',
        'ar' => 'شكراً لتسجيلك! قبل البدء، هل يمكنك تأكيد عنوان بريدك الإلكتروني بالنقر على الرابط الذي أرسلناه لك للتو؟ إذا لم تستلم البريد الإلكتروني، فسنرسل لك آخر بكل سرور.',
    ],
    'verify_email_link_sent' => [
        'en' => 'A new verification link has been sent to the email address you provided during registration.',
        'ar' => 'تم إرسال رابط تحقق جديد إلى عنوان البريد الإلكتروني الذي قدمته أثناء التسجيل.',
    ],
    'resend_verification_email' => [
        'en' => 'Resend Verification Email',
        'ar' => 'إعادة إرسال بريد التحقق',
    ],
    'Registration successful! Please check your email to verify your account before logging in.' => [
        'en' => 'Registration successful! Please check your email to verify your account before logging in.',
        'ar' => 'تم التسجيل بنجاح! يرجى التحقق من بريدك الإلكتروني لتأكيد حسابك قبل تسجيل الدخول.',
    ],

    // Social login (Google, X/Twitter) and complete profile
    'Continue with Google' => [
        'en' => 'Continue with Google',
        'ar' => 'متابعة باستخدام Google',
    ],
    'Continue with X' => [
        'en' => 'Continue with X',
        'ar' => 'متابعة باستخدام X',
    ],
    'Google login is not available.' => [
        'en' => 'Google login is not available.',
        'ar' => 'تسجيل الدخول عبر Google غير متاح.',
    ],
    'Twitter login is not available.' => [
        'en' => 'Twitter login is not available.',
        'ar' => 'تسجيل الدخول عبر X غير متاح.',
    ],
    'Social login is temporarily unavailable. Please try again later.' => [
        'en' => 'Social login is temporarily unavailable. Please try again later.',
        'ar' => 'تسجيل الدخول عبر الشبكات الاجتماعية غير متاح مؤقتاً. يرجى المحاولة لاحقاً.',
    ],
    'Session expired. Please try again.' => [
        'en' => 'Session expired. Please try again.',
        'ar' => 'انتهت الجلسة. يرجى المحاولة مرة أخرى.',
    ],
    'Unable to sign in with Google. Please try again.' => [
        'en' => 'Unable to sign in with Google. Please try again.',
        'ar' => 'تعذر تسجيل الدخول عبر Google. يرجى المحاولة مرة أخرى.',
    ],
    'Unable to sign in with X. Please try again.' => [
        'en' => 'Unable to sign in with X. Please try again.',
        'ar' => 'تعذر تسجيل الدخول عبر X. يرجى المحاولة مرة أخرى.',
    ],
    'We could not get your email from Google. Please use email/password or ensure your Google account has an email.' => [
        'en' => 'We could not get your email from Google. Please use email/password or ensure your Google account has an email.',
        'ar' => 'لم نتمكن من الحصول على بريدك من Google. يرجى استخدام البريد/كلمة المرور أو التأكد من ربط بريد بحساب Google.',
    ],
    'We could not sign you in with X. Please try again or use email/password.' => [
        'en' => 'We could not sign you in with X. Please try again or use email/password.',
        'ar' => 'تعذر تسجيل دخولك عبر X. يرجى المحاولة مرة أخرى أو استخدام البريد/كلمة المرور.',
    ],
    'Complete your profile' => [
        'en' => 'Complete your profile',
        'ar' => 'أكمل ملفك الشخصي',
    ],
    'Add your email' => [
        'en' => 'Add your email',
        'ar' => 'أضف بريدك الإلكتروني',
    ],
    'You signed in with X. To complete your account, please enter your email address.' => [
        'en' => 'You signed in with X. To complete your account, please enter your email address.',
        'ar' => 'سجّلت الدخول عبر X. لإكمال حسابك، يرجى إدخال بريدك الإلكتروني.',
    ],
    'Save and continue' => [
        'en' => 'Save and continue',
        'ar' => 'حفظ ومتابعة',
    ],
    'This email is already registered. Use another or sign in with that account.' => [
        'en' => 'This email is already registered. Use another or sign in with that account.',
        'ar' => 'هذا البريد مسجّل مسبقاً. استخدم بريداً آخر أو سجّل الدخول بهذا الحساب.',
    ],
    'Your email has been saved.' => [
        'en' => 'Your email has been saved.',
        'ar' => 'تم حفظ بريدك الإلكتروني.',
    ],
];

